...
void __USER_TEXT gpioer_config(struct gpio_cfg *cfg)
{
...
void __USER_TEXT gpioer_config_input(uint8_t port, uint8_t pin, uint8_t pupd)
{
	struct  gpio_cfg cfg = {
		.port = port,
		.pin = pin,
...
void __USER_TEXT gpioer_out_low(uint8_t port, uint8_t pin)
{
	*GPIO_ODR(port) &= ~(1 << pin);
}

uint8_t __USER_TEXT gpioer_input_bit(uint8_t port, uint8_t pin)
{
...
void __USER_TEXT gpioer_out(uint8_t port, uint8_t pin, uint8_t action) {
	if (action == GPIO_HIGH) {
		gpioer_out_high(port, pin);
	} else {
		gpioer_out_low(port, pin);
	}
}
