...
void __USER_TEXT fmc_sdram_config(struct fmc_sdram_cfg *cfg)
{
    ...
    tmpr1 = (uint32_t) cfg->column_bits_number | cfg->row_bits_number |
            cfg->sdmemory_data_width | cfg->internal_bank_number |
            cfg->cas_latency | cfg->write_protection | cfg->sd_clock_period |
            cfg->readburst | cfg->readpipe_delay;

    if (cfg->bank == FMC_Bank1_SDRAM) {
        *FMC_Bank5_6_SDCR(cfg->bank) = tmpr1;
    } else {
        tmpr3 = (uint32_t) cfg->sd_clock_period | cfg->readburst |
                cfg->readpipe_delay;
