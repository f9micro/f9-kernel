...
#define GPIO_BASE(port) \
    (APB2PERIPH_BASE + 0x0800 + (0x400 * port)) /* GPIO Port base address */
...
#define RCC_BASE \
    (AHBPERIPH_BASE + 0x1000) /* Reset and Clock Control base address */
#define FLASH_R_BASE \
...
#define GPIO_CRL(port)                       \
    (volatile uint32_t *) (GPIO_BASE(port) + \
                           0x00) /* Port configuration low register */
#define GPIO_CRH(port)                       \
    (volatile uint32_t *) (GPIO_BASE(port) + \
                           0x04) /* Port configuration high register */
#define GPIO_IDR(port)                       \
    (volatile uint32_t *) (GPIO_BASE(port) + \
                           0x08) /* Port input data register */
#define GPIO_ODR(port)                       \
    (volatile uint32_t *) (GPIO_BASE(port) + \
                           0x0C) /* Port output data register */
#define GPIO_BSRR(port)                      \
    (volatile uint32_t *) (GPIO_BASE(port) + \
                           0x10) /* Port bit set/reset register */
#define GPIO_BRR(port)                                                        \
    (volatile uint32_t *) (GPIO_BASE(port) + 0x14) /* Port bit reset register \
                                                    */
...
#define RCC_CFGR \
    (volatile uint32_t *) (RCC_BASE + 0x04) /* Clock Configuration Register */
#define RCC_CIR \
    (volatile uint32_t *) (RCC_BASE + 0x08) /* Clock Interrupt Register */
#define RCC_APB2RSTR \
    (volatile uint32_t *) (RCC_BASE + 0x0C) /* APB2 reset Register */
#define RCC_APB1RSTR \
    (volatile uint32_t *) (RCC_BASE + 0x10) /* APB1 reset Register */
#define RCC_AHBENR \
    (volatile uint32_t *) (RCC_BASE + 0x14) /* AHB Enable Register */
#define RCC_APB2ENR                   \
    (volatile uint32_t *) (RCC_BASE + \
                           0x18) /* APB2 Peripheral Clock Enable Register */
#define RCC_APB1ENR                   \
    (volatile uint32_t *) (RCC_BASE + \
                           0x1C) /* APB1 Peripheral Clock Enable Register */
#define RCC_BDCR                      \
    (volatile uint32_t *) (RCC_BASE + \
                           0x20) /* RCC Backup Domain Control Register */
#define RCC_CSR                       \
    (volatile uint32_t *) (RCC_BASE + \
                           0x24) /* RCC Clock Control & Status Register */
#define RCC_AHBRSTR \
    (volatile uint32_t *) (RCC_BASE + 0x28) /* AHB reset Register */
...
#define DMA_LISR                                                             \
    (volatile uint32_t *) (DMA_BASE + 0x00) /* DMA interrupt status register \
                                             */
#define DMA_IFCR                      \
...
#define DMA_CR_C(n)                          \
    (volatile uint32_t *) (DMA_BASE + 0x08 + \
                           (0x14 *           \
                            n)) /* DMA stream n configuration register */
#define DMA_NDTR_C(n)                        \
    (volatile uint32_t *) (DMA_BASE + 0x0C + \
                           (0x14 *           \
                            n)) /* DMA stream n number of data register */
#define DMA_PAR_C(n)                         \
    (volatile uint32_t *) (DMA_BASE + 0x10 + \
                           (0x14 *           \
                            n)) /* DMA stream n peripheral address register */
#define DMA_MAR_C(n)                         \
    (volatile uint32_t *) (DMA_BASE + 0x14 + \
                           (0x14 *           \
                            n)) /* DMA stream n memory 0 address register */
...
#define RCC_CFGR_PPRE1_M \
    (uint32_t) (0x7 << 8) /* APB low speed prescaler mask */
#define RCC_CFGR_PPRE1_DIV1 \
    (uint32_t) (0x0 << 8) /* APB low speed prescaler - HCLK/1 */
#define RCC_CFGR_PPRE1_DIV2 \
    (uint32_t) (0x4 << 8) /* APB low speed prescaler - HCLK/2 */
#define RCC_CFGR_PPRE1_DIV4 \
    (uint32_t) (0x5 << 8) /* APB low speed prescaler - HCLK/4 */
#define RCC_CFGR_PPRE1_DIV8 \
    (uint32_t) (0x6 << 8) /* APB low speed prescaler - HCLK/8 */
#define RCC_CFGR_PPRE1_DIV16 \
    (uint32_t) (0x7 << 8) /* APB low speed prescaler - HCLK/16 */
#define RCC_CFGR_PPRE2_M \
    (uint32_t) (0x38 << 8) /* APB high speec prescaler mask */
#define RCC_CFGR_PPRE2_DIV1 \
    (uint32_t) (0x0 << 8) /* APB high speed prescaler - HCLK/1 */
#define RCC_CFGR_PPRE2_DIV2 \
    (uint32_t) (0x20 << 8) /* APB high speed prescaler - HCLK/2 */
#define RCC_CFGR_PPRE2_DIV4 \
    (uint32_t) (0x28 << 8) /* APB high speed prescaler - HCLK/4 */
#define RCC_CFGR_PPRE2_DIV8 \
    (uint32_t) (0x30 << 8) /* APB high speed prescaler - HCLK/8 */
#define RCC_CFGR_PPRE2_DIV16 \
    (uint32_t) (0x38 << 8) /* APB high speed prescaler - HCLK/16 */
...
#define RCC_APB2RSTR_AFIORST \
    (uint32_t) (1 << 0) /* Alternate Function I/O reset */
#define RCC_APB2RSTR_IOPARST \
    (uint32_t) (1 << 2) /* Alternate Function I/O port A reset */
#define RCC_APB2RSTR_IOPBRST \
    (uint32_t) (1 << 3) /* Alternate Function I/O port B reset */
#define RCC_APB2RSTR_IOPCRST \
    (uint32_t) (1 << 4) /* Alternate Function I/O port C reset */
#define RCC_APB2RSTR_IOPDRST \
    (uint32_t) (1 << 5) /* Alternate Function I/O port D reset */
#define RCC_APB2RSTR_IOPERST \
    (uint32_t) (1 << 6) /* Alternate Function I/O port E reset */
...
#define RCC_AHBENR_SRAMEN                                                     \
    (uint16_t) (1 << 2)                        /* SRAM interface clock enable \
                                                */
...
#define RCC_APB1ENR_BKPEN \
    (uint32_t) (1 << 27) /* Backup interface clock enable */
#define RCC_APB1ENR_PWREN \
    (uint32_t) (1 << 28) /* Power interface clock enable */

#define RCC_APB2ENR_AFIOEN \
    (uint32_t) (1 << 0) /* Alternate Function I/O clock enable */
#define RCC_APB2ENR_IOPAEN \
    (uint32_t) (1 << 2) /* Alternate Function I/O port A clock enable */
#define RCC_APB2ENR_IOPBEN \
    (uint32_t) (1 << 3) /* Alternate Function I/O port B clock enable */
#define RCC_APB2ENR_IOPCEN \
    (uint32_t) (1 << 4) /* Alternate Function I/O port C clock enable */
#define RCC_APB2ENR_IOPDEN \
    (uint32_t) (1 << 5) /* Alternate Function I/O port D clock enable */
#define RCC_APB2ENR_IOPEEN \
    (uint32_t) (1 << 6) /* Alternate Function I/O port E clock enable */
#define RCC_APB2ENR_ADC1EN \
    (uint32_t) (1 << 9) /* ADC 1 interface clock enable */
#define RCC_APB2ENR_ADC2EN \
    (uint32_t) (1 << 10) /* ADC 2 interface clock enable */
...
#define GPIO_MODE_OUT_ALT_PP \
    (uint32_t) (0x06) /* Output alternate function push-pull mode */
#define GPIO_MODE_OUT_ALT_OD \
    (uint32_t) (0x07) /* Output alternate function open-drain mode */
...
/* DMA */
#define DMA_ISR_GIF(n)                                                       \
    (uint32_t) (0x1 << ((n - 1) * 4)) /* DMA Channel n Global interrupt flag \
                                       */
#define DMA_ISR_TCIF(n)                                                       \
    (uint32_t) (0x2 << ((n - 1) * 4)) /* DMA Channel n Transfer Complete flag \
                                       */
#define DMA_ISR_HTIF(n) \
    (uint32_t) (0x4 << ((n - 1) * 4)) /* DMA Channel n Half Transfer flag */
#define DMA_ISR_TEIF(n) \
    (uint32_t) (0x8 << ((n - 1) * 4)) /* DMA Channel n Transfer Error flag */

#define DMA_IFCR_CGIF(n)                                                      \
    (uint32_t) (0x1 << ((n - 1) * 4)) /* DMA Channel n Global interrupt clear \
                                       */
#define DMA_IFCR_CTCIF(n)         \
    (uint32_t) (0x2 << ((n - 1) * \
                        4)) /* DMA Channel n Transfer Complete clear */
#define DMA_IFCR_CHTIF(n) \
    (uint32_t) (0x4 << ((n - 1) * 4)) /* DMA Channel n Half Transfer clear */
#define DMA_IFCR_CTEIF(n) \
    (uint32_t) (0x8 << ((n - 1) * 4)) /* DMA Channel n Transfer Error clear */

#define DMA_CCR_EN (uint16_t) (1 << 0) /* DMA Channel enable */
#define DMA_CCR_TCIE \
    (uint16_t) (1 << 1) /* DMA Channel transfer complete interrupt enable */
#define DMA_CCR_HTIE \
    (uint16_t) (1 << 2) /* DMA Channel half transfer interrupt enable */
#define DMA_CCR_TEIE                                                         \
    (uint16_t) (1 << 3) /* DMA Channel transfer error interrupt error enable \
                         */
#define DMA_CCR_DIR \
    (uint16_t) (1 << 4) /* DMA Channel data transfer direction */
#define DMA_CCR_CIRC (uint16_t) (1 << 5) /* DMA Channel circular mode */
#define DMA_CCR_PINC \
    (uint16_t) (1 << 6) /* DMA Channel peripheral increment mode */
#define DMA_CCR_MINC                                                         \
...
#define DMA_CCR_MEM2MEM \
    (uint16_t) (1 << 14) /* DMA Channel Memory to memory mode */
