...
void test_ipc_basic(void)
{
	...
	ok = receiver_done &&
	     received_value == IPC_MAGIC_VALUE &&
	     received_label == IPC_MAGIC_LABEL &&
	     received_from == sender_tid.raw;
...
void test_ipc_multiword(void)
{
	...
	ok = multi_receiver_done &&
	     multi_word_count >= 4 &&
	     multi_words[0] == 0x11111111 &&
	     multi_words[1] == 0x22222222 &&
	     multi_words[2] == 0x33333333 &&
	     multi_words[3] == 0x44444444 &&
	     multi_label == IPC_MULTI_LABEL &&
	     multi_from == mw_sender.raw;

	if (ok) {
		TEST_PASS("ipc_multiword");
	} else {
		printf("Multi-word IPC failed: count=%lu label=0x%lx\n",
		       (unsigned long)multi_word_count,
		       (unsigned long)multi_label);
		TEST_FAIL("ipc_multiword");
	}
}
