...
/* colours */
static void set_colour(const char *col)
{
#ifdef USE_ANSI
    printf("%s", col);
#endif
...
void print_uline(const char *msg, char c)
{
...
__USER_TEXT void print_result(const char *str, bool test)
{
    printf("  %s: ", str);
...
__USER_TEXT L4_Word_t safe_mem_touch(void *addr)
{
...
__USER_TEXT void *get_pages(L4_Word_t count, int touch)
{
    void *ret = free_page;
...
__USER_TEXT void *get_new_page(void)
{
    return get_pages(1, 0);
}

__USER_TEXT void get_startup_values(void (*func)(void),
                                    L4_Word_t *ip,
                                    L4_Word_t *sp,
