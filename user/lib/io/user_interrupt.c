...
static void *__interrupt_handler_thread(void *arg)
{
    do {
        ...
        L4_Word_t action;
        L4_ThreadId_t recv_tid = {.raw = TID_TO_GLOBALID(THREAD_INTERRUPT)};
...
__USER_TEXT
static void __irq_msg(L4_Msg_t *out_msg,
                      L4_ThreadId_t tid,
                      unsigned int irq,
                      irq_handler_t handler,
                      L4_Word_t action,
                      uint16_t priority)
{
    ...
    L4_MsgPut(out_msg, USER_INTERRUPT_LABEL, IRQ_IPC_MSG_NUM, irq_data, 0,
              NULL);
}
...
{
    L4_MsgTag_t ret;
    L4_ThreadId_t send_tid = {.raw = TID_TO_GLOBALID(THREAD_IRQ_REQUEST)};
