...
{
    L4_Msg_t msg;
    L4_Word_t page[2] = {(base & 0xFFFFFFC0) | 0xA, size & 0xFFFFFFC0};
...
void __USER_TEXT __root_thread(kip_t *kip_ptr, utcb_t *utcb_ptr)
{
    ...
    for (user_struct *ptr = user_runtime_start; ptr != user_runtime_end;
         ++ptr) {
        L4_ThreadId_t tid;
        L4_Word_t stack;
        ...
        while (fpage->base || fpage->size) {
            if (fpage->base) {
                ...
                free_mem = (char *) (((L4_Word_t) free_mem + fpage->size - 1) &
                                     align_mask);
