...
class hook:
    ...
    def parse_from_obj(self, f):
        ...
        s = subprocess.check_output(
            ["%sobjdump" % (self.config.CROSS_COMPILE), "-s", "-j", ".init_hook", path]
        ).decode("utf-8")
...
def print_by_level(hooks):
    ...
    for level_tag, level_int in HOOK_LEVEL.items():
        ...
        token = False
        for hook in hooks:
            if hook.hook_level_int > last_level and hook.hook_level_int <= level_int:
                token = True
                print(hook)
...
if __name__ == "__main__":
    ...
    # Get grep output
    grep_output = filter(
        lambda x: x, subprocess.check_output(grep_command).decode("utf-8").split("\n")
    )
