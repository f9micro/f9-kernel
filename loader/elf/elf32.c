...
int elf32_checkFile(struct Elf32_Header *file)
{
    if (file->e_ident[EI_MAG0] != ELFMAG0 ||
        ...
        file->e_ident[EI_MAG2] != ELFMAG2 || file->e_ident[EI_MAG3] != ELFMAG3)
...
void elf32_printStringTable(struct Elf32_Header *elfFile)
{
    ...
    for (counter = 0; counter < sections[elfFile->e_shstrndx].sh_size;
         counter++) {
...
void elf32_getSegmentInfo(struct Elf32_Header *elfFile,
                          int segment,
                          uint64_t *p_vaddr,
                          uint64_t *p_addr,
                          uint64_t *p_filesz,
