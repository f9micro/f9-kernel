...
void *elf_getSectionNamed(void *elfFile, char *_str)
{
...
uint32_t elf_loadFile(void *elfFile)
{
    ...
    for (i = 0; i < num_pheaders; i++) {
        ...
        src = (uint32_t) (uintptr_t) elfFile +
              elf_getProgramHeaderOffset(elfFile, i);
        dbg_printf(DL_BASIC, "Elf program header offset: %p\n", src);
        dbg_printf(
            DL_BASIC, "Copying to range from 0x%x to 0x%x of size: 0x%x\n",
            (unsigned int) dest, (unsigned int) dest + (unsigned int) len,
            (unsigned int) len);
