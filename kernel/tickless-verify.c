...
void tickless_verify_stop(uint32_t ktimer_now)
{
    ...
    verify_records[verify_n].need =
        verify_start_need * CONFIG_KTIMER_HEARTBEAT - verify_start_systick;
...
int32_t tickless_verify_stat(int *times)
{
    ...
    for (int i = 0; i < n; i++) {
        int32_t hwtimer_need_diff =
            verify_records[i].hwtimer_diff - verify_records[i].need;
#ifdef CONFIG_KDB
        int32_t hwtimer_ktimer_diff =
            verify_records[i].hwtimer_diff - verify_records[i].ktimer_diff;

        dbg_printf(
            DL_KDB,
            "%c Record %2d: %10d | %10d | %10d | %c%9d | %c%9d | %5d(%d)\n",
            i == verify_n - 1 ? '*' : ' ', i, verify_records[i].need,
            verify_records[i].hwtimer_diff, verify_records[i].ktimer_diff,
            hwtimer_need_diff >= 0 ? ' ' : '-',
            hwtimer_need_diff >= 0 ? hwtimer_need_diff : -hwtimer_need_diff,
            hwtimer_ktimer_diff >= 0 ? ' ' : '-',
            hwtimer_ktimer_diff >= 0 ? hwtimer_ktimer_diff
                                     : -hwtimer_ktimer_diff,
            verify_records[i].count, verify_records[i].count_int);
#endif /* CONFIG_KDB */
