...
static void create_fpage_chain(memptr_t base,
                               size_t size,
                               int mpid,
                               fpage_t **pfirst,
                               fpage_t **plast)
{
    ...
    while (size) {
        ...
        if (!newfp) {
            ...
            dbg_printf(DL_KDB, "FPAGE: chain alloc failed at base=%p\n", base);
            return;
        }
...
fpage_t *split_fpage(as_t *as, fpage_t *fpage, memptr_t split, int rl)
{
    ...
    create_fpage_chain(base, (split - base), fpage->fpage.mpid, &lfirst,
                       &llast);
    create_fpage_chain(split, (end - split), fpage->fpage.mpid, &rfirst,
                       &rlast);
...
int assign_fpages_ext(int mpid,
                      as_t *as,
                      memptr_t base,
                      size_t size,
                      fpage_t **pfirst,
        ...
        while (base < end && *fp) {
            if (base < FPAGE_BASE(*fp)) {
...

                {
                    ...
                    create_fpage_chain(abase, aend - abase, mpid, &first,
                                       &last);
                }
...
int map_fpage(as_t *src, as_t *dst, fpage_t *fpage, map_action_t action)
{
    ...
    if (!fpage || !dst) {
        dbg_printf(DL_KDB, "FPAGE: map_fpage NULL arg! fpage=%p dst=%p\n",
                   fpage, dst);
        return -1;
    }
    ...
    if (!fpmap) {
        dbg_printf(DL_KDB, "FPAGE: map alloc failed! src=%p dst=%p\n", src,
                   dst);
        return -1;
    }
    ...
    dbg_printf(DL_MEMORY, "MEM: %s fpage %p [b:%p sz:2**%d] from %p to %p\n",
               (action == MAP) ? "mapped" : "granted", fpmap, FPAGE_BASE(fpmap),
               fpmap->fpage.shift, src, dst);
...
int unmap_fpage(as_t *as, fpage_t *fpage)
{
    ...
    /* Find previous fpage in circular map list with cycle guard */
    {
        ...
        if (guard <= 0) {
            dbg_printf(DL_KDB, "FPAGE: cycle detected in unmap_fpage %p\n",
                       fpage);
            return -1;
        }
    }
